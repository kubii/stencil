...
function render(opts) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        hydrateHtml(config, ctx, registry, hydrateOpts, results, function () {
            if (results.diagnostics.length) {
                var msg = results.diagnostics.map(function (d) { return d.messageText; }).join('\n');
                reject(msg);
                return;
            }
            var rootElm = (results.root && results.root.children.length > 1 && results.root.children[1].firstElementChild) || null;
            if (rootElm) {
