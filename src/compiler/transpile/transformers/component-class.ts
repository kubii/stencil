    ...
    function visit(fileMeta: ModuleFile, node: ts.Node): ts.VisitResult<ts.Node> {
      switch (node.kind) {
        ...
        case ts.SyntaxKind.ClassDeclaration:
          return visitClass(config, fileMeta, diagnostics, node as ts.ClassDeclaration, false);
  ...
  return (transformContext) => {
    ...
    function visit(fileMeta: ModuleFile, node: ts.Node): ts.VisitResult<ts.Node> {
      switch (node.kind) {
        ...
        case ts.SyntaxKind.ClassDeclaration:
          return visitClass(config, fileMeta, diagnostics, node as ts.ClassDeclaration, true);
...
function visitClass(config: BuildConfig, fileMeta: ModuleFile, diagnostics: Diagnostic[], classNode: ts.ClassDeclaration, removeDecorator: boolean) {
  ...
  if (cmpMeta) {
    ...
    getElementDecoratorMeta(cmpMeta, classNode, removeDecorator);
